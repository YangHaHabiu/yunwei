...
func TestA(t *testing.T) {
	...
	path := "/admin/dict/type/host_role_type"
	split := strings.Split(path, "/")
	fmt.Println(len(split))
	path = filepath.Join("/", split[1], split[2], split[3])
	fmt.Println(path)
...
func TestSetString(t *testing.T) {
	c := treeset.NewWithIntComparator()
	c.Add(1)
...
func TestSetIntersection(t *testing.T) {
	{
		set := treeset.NewWithStringComparator()
		another := treeset.NewWithIntComparator()
		set.Add("a", "b", "c", "d")
		another.Add(1, 2, 3, 4)
	...
	set := treeset.NewWithStringComparator()
	another := treeset.NewWithStringComparator()
...
func benchmarkContains(b *testing.B, set *treeset.Set, size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			set.Contains(n)
		}
...
func benchmarkAdd(b *testing.B, set *treeset.Set, size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			set.Add(n)
		}
...
func benchmarkRemove(b *testing.B, set *treeset.Set, size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			set.Remove(n)
		}
