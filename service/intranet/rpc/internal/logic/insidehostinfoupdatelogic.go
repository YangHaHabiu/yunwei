...
func (l *InsideHostInfoUpdateLogic) InsideHostInfoUpdate(in *intranetclient.UpdateInsideHostInfoReq) (*intranetclient.InsideHostInfoCommonResp, error) {
	var tmp model.InsideHostInfo
	err := copier.Copy(&tmp, in.One)
	if err != nil {
		return nil, xerr.NewErrMsg("拷贝更新数据失败，原因：" + err.Error())
	}
	err = l.svcCtx.InsideHostInfoModel.Update(l.ctx, &tmp)
	if err != nil {
