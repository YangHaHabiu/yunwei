...
func (l *InsideTasksLogsUpdateLogic) InsideTasksLogsUpdate(in *intranetclient.UpdateInsideTasksLogsReq) (*intranetclient.InsideTasksLogsCommonResp, error) {
	var tmp model.InsideTasksLogs
	err := copier.Copy(&tmp, in.One)
	if err != nil {
		return nil, xerr.NewErrMsg("拷贝更新数据失败，原因：" + err.Error())
	}
	err = l.svcCtx.InsideTasksLogsModel.Update(l.ctx, &tmp)
	if err != nil {
