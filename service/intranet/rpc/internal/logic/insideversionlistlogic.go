...
func (l *InsideVersionListLogic) InsideVersionList(in *intranetclient.ListInsideVersionReq) (*intranetclient.ListInsideVersionResp, error) {
	var (
		count int64
		list  []*intranetclient.ListInsideVersionData
		all   *[]model.InsideVersionNew
		err   error
	)
	...
	count, _ = l.svcCtx.InsideVersionModel.Count(l.ctx, filters...)
	if in.PageSize == 0 && in.Current == 0 {
		all, err = l.svcCtx.InsideVersionModel.FindAll(l.ctx, filters...)
	} else {
		all, err = l.svcCtx.InsideVersionModel.FindPageListByPage(l.ctx, in.Current, in.PageSize, filters...)
	}
	if err != nil {
	...
	return &intranetclient.ListInsideVersionResp{
		Rows:  list,
		Total: count,
	}, nil
}
