...
func (l *InsideTasksAddLogic) InsideTasksAdd(in *intranetclient.AddInsideTasksReq) (*intranetclient.InsideTasksCommonResp, error) {
	var tmp model.InsideTasks
	err := copier.Copy(&tmp, in.One)
	if err != nil {
		return nil, xerr.NewErrMsg("拷贝新增数据失败，原因：" + err.Error())
	}
	_, err = l.svcCtx.InsideTasksModel.Insert(l.ctx, &tmp)
	if err != nil {
