...
func (l *InsideProjectClusterUpdateLogic) InsideProjectClusterUpdate(req *types.UpdateInsideProjectClusterReq) error {
	var tmp intranet.InsideProjectClusterCommon
	err := copier.Copy(&tmp, req)
	if err != nil {
		return xerr.NewErrMsg("更新拷贝数据失败，原因：" + err.Error())
	}
	_, err = l.svcCtx.IntranetRpc.InsideProjectClusterUpdate(l.ctx, &intranetclient.UpdateInsideProjectClusterReq{One: &tmp})
	if err != nil {
		return err
	}
	return nil
